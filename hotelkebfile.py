...
# Function to write all people details to a text file
def write_people_details_to_file(filename, people_distribution):
    ...
    with open(filename, 'w', encoding='utf-8') as file:
        ...
        for person_number in range(1, total_people + 1):
            result = find_position(person_number, people_distribution)
            # Write the result to the file
            file.write(f'Person {person_number}: {result}\n')
    
    print(f'All people details have been written to {filename}')


# Function to get input and run the process
def run_calculation_and_write_to_file():
    ...
    write_people_details_to_file(filename, people_distribution)

# Uncomment the line below to run the function when using in a terminal
run_calculation_and_write_to_file()
